...
Satellites = 4; %default is 4 in scenario 4

SIM_ITER = 500; %500, default value

% First Iteration
[SeparationUT, PhaseError, AmplitudeError] = TidesCode_NonMigrating_Function_Scen4(Satellites);
...
for k=1:dim_matrix(1)
    semilogy(SeparationUT,AmplitudeError(k,:),style{k},'linewidth',2);    
end
s(2) = subplot(1,2,2); 
hold on
for k=1:dim_matrix(1)
    semilogy(SeparationUT,PhaseError(k,:),style{k},'linewidth',2);    
end

xlabel(s(1),'Universal Time Separation Between Satellites');
